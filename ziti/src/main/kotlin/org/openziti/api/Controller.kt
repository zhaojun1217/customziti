...
internal class Controller(endpoint: URL, sslContext: SSLContext, trustManager: X509TrustManager) :
    ...
    internal interface API {
        ...
        ): Deferred<Response<Collection<Session>>>
        ...
        @DELETE("{p}")
        fun delete(
            @Header("zt-session") session: String,
            @Path("p", encoded = true) path: String
        ): Call<Response<Unit>>
    }
    ...
    internal val loggingInterceptor = HttpLoggingInterceptor().apply {
        val level =
            HttpLoggingInterceptor.Level.valueOf(System.getProperty("ZitiControllerDebug", "NONE"))
        setLevel(level)
    }
    ...
    init {
        clt = OkHttpClient.Builder().apply {
            ...
            dns { mutableListOf(getIpv6Address(it)) }
        }.build()
    ...
    private fun getIpv6Address(hostname: String): InetAddress? {
        try {
            // IPv6 地址字符串
            if ("www.idn-ctrl.com".equals(hostname)) {
                return InetAddress.getByName("2408:8631:c02:ffd2::66")
            } else if ("www.p-router.com".equals(hostname)) {
                return InetAddress.getByName("2408:8631:c02:ffd2::11")
            } else {
    ...
    internal fun createIdentity(
        name: String,
        type: String = "Device",
        enrollment: String = "ott"
    ): Id {
    ...
    internal fun getServices() = pagingRequest { offset ->
        api.getServicesAsync(offset = offset, limit = 25)
    }
    ...
    internal fun getServiceTerminators(s: Service): Flow<ServiceTerminator> =
        pagingRequest { offset ->
            api.getServiceTerminators(s.id, offset = offset)
        }
